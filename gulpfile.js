...
gulp.task('browser-sync', ['nodemon'], function () { // for more browser-sync config options: http://www.browsersync.io/docs/options/
  browserSync.init({
    ...
    port: 5000 // informs browser-sync to use the following port for the proxied app. notice that the default port is 3000, which would clash with our expressjs
  });
});
